...
export function UserPresence() {
  ...
  const refreshOthers = async () => {
    ...
    // Get all users actively in sessions (hosts, co-hosts, and followers)
    const usersInSessions = new Set();

    Object.entries(sessions).forEach(([hostId, session]) => {
      // Only add host if they have followers or co-hosts (active session)
      if ((session.followers && session.followers.length > 0) ||
        (session.coHosts && session.coHosts.length > 0)) {
        usersInSessions.add(hostId);
      }

      // Add followers
      (session.followers || []).forEach(fid => usersInSessions.add(fid));

      // Add co-hosts
      (session.coHosts || []).forEach(chid => usersInSessions.add(chid));
    });

    // Add self only if actively participating
    if (selfIdLocal && (isHost || following)) {
      usersInSessions.add(selfIdLocal);
    }

    // Filter to only show users in sessions
    let usersToShow = Array.from(usersInSessions);

    // Sort: hosts first, then others
    usersToShow.sort((a, b) => {
      const aIsHost = !!sessions[a];
      const bIsHost = !!sessions[b];
      if (aIsHost && !bIsHost) return -1;
      if (!aIsHost && bIsHost) return 1;
      return String(a).localeCompare(String(b));
    });

    const mapped = usersToShow
      .map((userId) => {
        const { iconIndex, colorIndex } = getOrSetVisualInTags(userId);
        const isCurrentHost = !!sessions[userId];
        const isSelf = userId === selfIdLocal;
        return { remoteId: userId, iconIndex, colorIndex, isCurrentHost, isSelf };
      });
  ...
  const startSession = () => {
    ...
    const notifyOthers = async () => {
      ...
      if (others.length > 0) {
        sendRemoteData(others, "sessionStarted", { hostId: selfId });
      }
    };
    notifyOthers();
  };
      ...
      >
        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
          ...
          {visibleUsers.map(({ remoteId, iconIndex, colorIndex }, index) => {
            ...
            const { inSession, role, config } = getUserSessionInfo(remoteId)
            if (role !== 'none')
              return (
  ...
  return (
    ...
    >
      <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 12 }}>
        ...
        <div>
          <p style={{ fontSize: 13 }}>
            Following <b style={{ color }}>{hostId}</b>
          </p>
      ...
      <div style={{ marginTop: 10 }}>
        ...
        <div style={{ display: "flex", flexWrap: "wrap", marginTop: 6 }}>
          {[4, 8, 16, 20].map((d) => (
            ...
            >
              {d === 4 ? <span class="material-symbols-outlined">
                all_inclusive
              </span> : `${d} sec`}
            </button>
          ))}
