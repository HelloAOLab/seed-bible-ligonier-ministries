...
function SettingsPanel({ icon, name, address, installed, setUpdate, data }) {
    ...
    const OPTIONS = {
        type: 'normal', items: [
            {
                icon: <MenuIcon name={installed ? "extension_off" : "download"} />, title: installed ? 'Uninstall' : 'Install', onClick: async () => {
                    if (!installed) {
            ...
            <div style={containerStyle}>
                <div style={headerStyle}>
                    ...
                    <div style={headerRightStyle}>
                        {installed && <ToggleSwitch
                            isOn={buttonEnabled}
                            onToggle={() => { setbuttonEnabled(!buttonEnabled); globalThis.ToggleToolActive(name, 'stop') }}
                        />}
                        <span onClick={() => openPopupSettings(OPTIONS)} className="material-symbols-outlined" style={iconStyle}>more_vert</span>
                        {installed && <span onCLick={() => setExpand(prev => !prev)} className="material-symbols-outlined" style={{ cursor: 'pointer' }} style={iconStyle}>{expand ? "expand_less" : 'expand_more'}</span>}                    </div>
                </div>

                {expand && <div style={sectionStyle}>
                    <div style={optionStyle}>
                        ...
                        <ToggleSwitch
                            ...
                            onToggle={() => { globalThis.ToggleToolActive(name); setShowInToolbar(!showInToolbar) }}
                    ...
                    <div style={optionStyle}>
                        <div style={optionLabelStyle}>
                            <span className="material-symbols-outlined" style={iconStyle}>view_sidebar</span>
                            <span>Panel</span>
                        </div>
                        <ToggleSwitch
                            isOn={globalThis.IsToolSraterToolbar && globalThis.IsToolSraterToolbar(name)}
                            onToggle={() => { globalThis.ToToggleShowInStarterToolbar(name); setShowInPanel(!showInPanel) }}
                            disabled={!buttonEnabled}
                        />
                    </div>
                    ...
                    <div style={lastOptionStyle}>
                        <div style={optionLabelStyle}>
                            <span className="material-symbols-outlined" style={iconStyle}>description</span>
                            <span>Below the page</span>
                        </div>
                        <ToggleSwitch
                            isOn={globalThis.IsToolInPageToolbar && globalThis.IsToolInPageToolbar(name)}
                            onToggle={() => { globalThis.ToToggleShowInPageToolbar(name); setShowBelowPage(!showBelowPage) }}
                            disabled={!buttonEnabled}
                        />
                    </div>
                </div>}
    ...
    return (
        <div className="extensions-container">
            ...
            <div className="routerTitle blackText">
                <div className="blackText"> <MenuIcon name="extension" /></div>
                <div>Extensions</div>
            </div>
