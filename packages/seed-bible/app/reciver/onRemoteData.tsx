...
if (!iFollowRemote && !isMyFollower) {
    if (name === "updateSharingData") {
        return shout("updatedYourData", { user: remoteId, tab: { ...(payload || {}) } });
    }
    if (name === "personLeftTheChat") {
        return shout("onPersonLeftRemote", { user: remoteId, tab: { ...(payload || {}) } });
    }
...
const followers = Array.isArray(hostSession?.followers) ? hostSession.followers : [];
...
const forwardToFollowersExcept = (excludeId, eventName, data) => {
    ...
    if (targets.length) sendRemoteData(targets, eventName, { ...data, senderId: selfId });
};
...
switch (name) {
    ...
    case "highlight": {
        ...
        shout("remoteHighlightChange", { ...(payload || {}), scroll: allowAutoScroll, fadeIn: highlightDuration });
    ...
    case "appClick": {
        ...
        if (appName && globalThis[appName]?.onClick) {
            try { globalThis[appName].onClick(); } catch (e) { os.log("appClick error", e); }
        }
        if (iAmHost && remoteId !== hostId) {
    ...
    case "playlistQueueUpdated": {
        ...
        shout("remotePlaylistMetaDataUpdate", { ...(payload || {}), playlistUpdated: true });
        if (iAmHost && remoteId !== hostId) {
    ...
    case "playlistCurrentIndexUpdate": {
        ...
        shout("remotePlaylistMetaDataUpdate", { ...(payload || {}), indexesUpdate: true });
        if (iAmHost && remoteId !== hostId) {
            forwardToFollowersExcept(remoteId, "playlistCurrentIndexUpdate", payload || {});
        }
        return;
    }
    ...
    case "personLeftTheChat": {
        shout("onPersonLeftRemote", { user: remoteId, tab: { ...(payload || {}) } });
        return;
    }
