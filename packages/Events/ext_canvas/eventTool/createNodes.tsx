...
const createSoureBot = async () => {
  ...
  for (let i = 0; i < uidData.length; i++) {
    ...
    if (uidData[0].type === "verse") {
      ...
      // console.log(`https://ligonier.helloao.org/api/ESV/${uidData[0].book[0].bookName}/${osisRef[1]}.json`)
      const bsbtranslation = await web
        .get(
          `https://ligonier.helloao.org/api/ESV/${uidData[0].book[0].bookName}/${osisRef[1]}.json`
        )
        .then((data) => {
          return data.data;
        });
