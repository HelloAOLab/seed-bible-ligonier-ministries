...
const DragDrop = ({ massAdd, attachLink, onGenClick = () => { }, setItemSelected = () => { }, itemSelected, access, isSomethingEmbededChecked, checkListEmbeded, setChecklistEmbeded, onDisembed = () => { }, layers = true, embedding, setEmbedding, setRef = {}, allowHeadingCheck, selectedTags, playlistName, currentDateActive, clickPass, currentFormat, readingPlanEnabled, linkingMode, viewOnly, checkListData = {}, oldItemsMap = {}, parentId, list, isPlayer, playingPlaylist, activeItemList = {}, activeItemID, toggleRemoved, setList, editDataFromPlaylist, playListSubId, setPlaylistFromRow = () => { }, playListSubIndex = null, deleteFromList = () => { }, onClickItem = () => { }, onClick = () => { }, creatingPlaylist = false, color, icon, isCustomColor, description, isCustomIcon }) => {
    ...
    const autoPlayToggle = (index, pId, id) => {
        setList(prev => {
            ...
            if (pIndex > -1) {
                const attachmentIndex = old[pIndex]?.additionalInfo?.layers?.findIndex(ele => ele.id === id);
                if (attachmentIndex === 0) {
                    old[pIndex].additionalInfo.layers[0].autoPlay = !old[pIndex].additionalInfo.layers[0].autoPlay;
                }
            }
            return old;
        });
                        ...
                        >
                            {data.type === "heading" && !playingPlaylist ?
                                <p
                                    ...
                                    onClick={(e) => {
                                        e.stopPropagation();
                                        globalThis.SetEditRichText?.({ id: data.id, text: data.content, parentID: id })
                                    }}
                                >
                                    <span class="material-symbols-outlined">
                                        edit
