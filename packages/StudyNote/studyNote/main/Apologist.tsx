...
function SgCard({ item, isOpen, onToggle, viewMode = "list" }) {
    ...
    return (
        <article className={`sg-card ${isBook ? 'sg-card-book-inline' : ''} ${viewMode === "grid" ? "sg-card-grid" : "sg-card-list"} ${isOpen ? "is-open" : ""}`}>
...
function ApologistSearch({
            ...
            try {
                ...
                const sortedResults = allResults.sort((a, b) => {
                    ...
                    return 0;
                       ...
                       const bookIds = sortedResults.filter(item => item.type === 'book').map(item => item.id);
    ...
    const loadMore = () => {
        ...
        setTimeout(() => {
            ...
            const newBookIds = newData.filter(item => item.type === 'book').map(item => item.id);
