...
import { Chapter } from "interactiveBible.managers.MapsManager.Chapter"
import { PresentUserPresenceBookIcon } from "interactiveBible.managers.MapsManager.PresentUserPresenceIcon"
...
export const Book = ({
    ...
    const {chaptersCount, shortName} = useMemo(() => { 
        return {
            chaptersCount: StacksManager.tags.booksStaticInfo[bookInfo.commonName].numberOfChapters,
            shortName: StacksManager.tags.booksStaticInfo[bookInfo.commonName].abbreviation
        } 
    }, [])
    const bookCoverHeight = useMemo(() => { return `${Math.round(thisBot.GetMapBookHeightByName({bookName: bookInfo.commonName})  * scaleFactor)}px` }, [scaleFactor]);
