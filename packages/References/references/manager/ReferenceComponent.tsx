...
const ReferenceComponent = ({ reference, handleRedirect }) => {
    ...
    const loadContent = useCallback(async ({ reference }) => {
        setRdLoading(true);
        let contentReq = await web.get(`https://bible.helloao.org/api/BSB/${reference.book}/${reference.chapter}.json`)
    ...
    return <>
        ...
        <div class="reference-container" style={{ boxShadow: "rgba(99, 99, 99, 0.2) 0px 2px 8px 0px", width: "250px", height: "fit-content" }}>
            <div class="reference-components">
                <span onClick={() => { handleRedirect({ reference }) }} class="reference-title">{`${tags.IdToName[reference?.book]} ${reference?.chapter}:${reference?.verse}`}</span>
                {
