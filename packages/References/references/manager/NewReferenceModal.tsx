...
const ReferenceModal = ({ reference }) => {
    ...
    const populateReferenceData = useCallback(async () => {
        ...
        const referenceArrayKey = `${reference.book}.${reference.chapter}.${reference.verse}`;    if (referenceBot.masks?.referenceDataObject && referenceBot.masks?.referenceDataObject[referenceArrayKey]) {
ull;
    ...
    useEffect(() => {
        setReferenceArray(showMore ? reference.references : reference.references.slice(0, 3));
    }, [reference, showMore]);
    ...
    return <>
        ...
        <div class="reference-container" onClick={e => e.stopPropagation()} onContextMenu={e => e.stopPropagation()} style={{ height: "400px", width: "250px", boxShadow: "rgba(99, 99, 99, 0.2) 0px 2px 8px 0px" }}>
            <div class="heading">
                ...
                <span onClick={() => {
                    globalThis.currentReference !== `${reference.book}.${reference.chapter}.${reference.verse}` && shout("ToggleReference", { book: tags.IdToName[reference.book], chapter: reference.chapter, verse: reference.verse });
                    closePopupSettings();
...

            {
                referenceArray.map(childReference => {
                    return <div class="reference-components">
                        <span onClick={(e) => { handleTitleContext({e, reference: childReference }) }} class="reference-title">{`${tags.IdToName[childReference.book]} ${childReference.chapter}:${childReference.verse}${childReference?.endVerse ? `-${childReference.endVerse}` : ""}`}</span>
                        {
            ...
            {reference.references.length > 3 && <div class="reference-components" style="cursor: pointer; justify-content: center; align-items: center;">
